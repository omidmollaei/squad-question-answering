...
def encoder_layer(hparams, name: str = "encoder_layer"):
    ...
    outputs = tf.keras.layers.Dense(hparams.num_units, activation=hparams.activation)(attention)
...
def encoder(hparams, name: str = "encoder"):
    ...
    embeddings = PositionalEncoding(position=hparams.vocab_size, d_model=hparams.d_model)(embeddings)
    outputs = tf.keras.layers.Dropout(hparams.dropout)(embeddings)
    for i in range(hparams.num_layers):
        outputs = encoder_layer(hparams, name=f"encoder_layer_{i}")([outputs, padding_mask])
    return tf.keras.Model(inputs=[inputs, padding_mask], outputs=outputs, name=name)
...
def decoder(hparams: Parameters, name: str = "decoder"):
    inputs = tf.keras.Input(shape=(None,), name="inputs")
    enc_outputs = tf.keras.Input(shape=(None, hparams.d_model), name="encoder_outputs")
    look_ahead_mask = tf.keras.Input(shape=(1, None, None), name="look_ahead_mask")
    padding_mask = tf.keras.Input(shape=(1, 1, None), name="padding_mask")
...
def transformer(hparams: Parameters, name: str = "transformer"):
    inputs = tf.keras.Input(shape=(None,), name="inputs")
    ...
    dec_outputs = decoder(hparams)(inputs=[dec_inputs, enc_outputs, look_ahead_mask, dec_padding_mask])
